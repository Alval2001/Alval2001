...
function setBox (e)
{
	if (document.getElementById(e).textContent == '' && win != 1)
	{
		if (hod == 'x') {
			...
			document.getElementById('who').innerHTML = 'Ход игрока 2'
			hodNumber+=1;
		}
		else{
			document.getElementById(e).append('o')
			document.getElementById('who').innerHTML = 'Ход игрока 1'
			hodNumber+=1;
		}
		
		if (checkWin() == true){
			if(hod == 'x'){
				document.getElementById('who').innerHTML = '<b>Игрок 1 победил!!!</b>'
				win = 1;
			}
			else{
				document.getElementById('who').innerHTML = '<b>Игрок 2 победил!!!</b>'
				win = 1;
			}
		} 
		if (checkWin() == false && hodNumber==10)
		{
			document.getElementById('who').innerHTML = '<b>Ничья</b>'
			win = 1;
		}
